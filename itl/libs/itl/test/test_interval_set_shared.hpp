...
void interval_set_fundamentals_4_ordered_types()
{
	...
	BOOST_CHECK_EQUAL(interval_set<T>().cardinality(), itl::neutron<size_T>::value());
	BOOST_CHECK_EQUAL(interval_set<T>().size(), itl::neutron<size_T>::value());
	BOOST_CHECK_EQUAL(interval_set<T>().interval_count(), 0);
	BOOST_CHECK_EQUAL(interval_set<T>().iterative_size(), 0);
