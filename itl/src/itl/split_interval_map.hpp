...
/*
template <typename DomainT, typename CodomainT, template<class>class Interval, template<class>class Compare, template<class>class Alloc>
void split_interval_map<DomainT,CodomainT,Interval,Compare,Alloc>::friends(split_interval_set<DomainT,interval_type>& frnds, const ItvT& x)
{
	iterator fst_it = this->_map.lower_bound(x);
	iterator end_it = this->_map.upper_bound(x);
	frnds.clear();

	if(fst_it==end_it)
		return;

	// There is a first interval
	interval_type leftResid, rightResid;
	ImplMapT::iterator it=fst_it, nxt_it=fst_it, victim;
	interval_type leftResid;  (*it).KEY_VALUE.left_surplus(leftResid, x);
	interval_type rightResid;

	// Collect all iterators if intervals, that overlap with x
	// Also collect the split_interval_set friends if those Intervals
	while(it!=end_it)
	{
		//JODO friendsIters.insert(it);
		frnds.insert((*it).KEY_VALUE);
		if((++nxt_it)==end_it) 
			(*it).KEY_VALUE.right_surplus(rightResid, x);
		it++;
	}
	...
	if(!leftResid.empty())
	{
		...
		// erstes Interval ragt links über x hinaus. Spalte fst_itv auf
		// Muss aber später gemacht werden
	}

}
*/
/* JODO make Testcode
   the code here is a possibility to implement insert quite abstract;
   could serve as test code.
...
void split_interval_map<DomainT,CodomainT,Interval,Compare,Alloc>::friends(split_interval_set<DomainT,interval_type>& frnds, const ItvT& x)
{
	...
	iterator it;
	frnds.clean();
	for(it=fst_it; it!=end_it; it++) frnds.insert((*it).KEY_VALUE);
}
...
{
	const interval_type& x_itv = x.KEY_VALUE;
	CodomainT      x_val = x.CONT_VALUE; // must be a copy

	if(x_itv.empty()) return;

	pair<ImplMapT::iterator,bool> insertion = this->_map.insert(x);

	if(!insertion.WAS_SUCCESSFUL)
	{

		split_interval_set<DomainT,interval_type> xSet(x_itv);
		split_interval_set<DomainT,interval_type> frnds;
		friends(frnds, x_itv);
		split_interval_set<DomainT,interval_type> redsids(frnds); resids   -= xSet ;
		split_interval_set<DomainT,interval_type> complem(xSet);  complem  -= frnds; 
		split_interval_set<DomainT,interval_type> intersec(xSet); intersec *= frnds;
		
		// resids: have been in the set already; keep their values
		// complem: have not been in the set; apply x_val
		// intersec: are in *this and in x; must be added
	}
}
*/
...
void split_interval_map<DomainT,CodomainT,Interval,Compare,Alloc>::subtract(const value_type& x)
{
	/*
#ifdef _DEBUG
	interval_set<DomainT,interval_type> clone; toItvSetT(clone);
#endif
	*/
...
//		const interval_type& cur_itv = (*it).KEY_VALUE ;
		CodomainT&     cur_val = (*it).CONT_VALUE ; //CodomainT::OP = ? JODO check
